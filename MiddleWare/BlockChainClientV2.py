...
class BlockChainConnection:
    ...
    def __update_with_proof(self,weights,bias,accountNR,Device_name, round,  fact):
        ...
        #weights1 = [[int(x) for x in y] for y in weights1]

        weights2 = weights["weights2"]
        #weights2 = [[int(x) for x in y] for y in weights2]

        weights3 = weights["weights3"]
        #weights3 = [[int(x) for x in y] for y in weights3]

        bias1 = bias["bias1"]
        #bias1 = [int(x) for x in bias1]

        bias2 = bias["bias2"]
        #bias2 = [int(x) for x in bias2]

        bias3 = bias["bias3"]
        #bias3 = [int(x) for x in bias3]
    ...
    def __update_without_proof(self,weights,bias,accountNR, Device_name, round):
        weights1 = weights["weights1"]
        weights1 = [[int(x) for x in y] for y in weights1]
        #weights1 = [[x for x in y] for y in weights1]
        weights2 = weights["weights2"]
        weights2 = [[int(x) for x in y] for y in weights2]
        #weights2 = [[x for x in y] for y in weights2]

        # weights3 = weights["weights3"]
        # weights3 = [[int(x) for x in y] for y in weights3]
        #weights3 = [[x for x in y] for y in weights3]

        bias1 = bias["bias1"]
        bias1 = [int(x) for x in bias1]
        #bias1 = [x for x in bias1]

        bias2 = bias["bias2"]
        bias2 = [int(x) for x in bias2]
        #bias2 = [x for x in bias2]

        # bias3 = bias["bias3"]
        # bias3 = [int(x) for x in bias3]
        #bias3 = [x for x in bias3]
    ...
    def get_Hidden_Layers_neurons(self, accountNR):
        Layer1 = self.FLcontractDeployed.functions.layer1Neurons().call(
            {"from": self.web3Connection.eth.accounts[accountNR]})
        Layer2 = self.FLcontractDeployed.functions.layer2Neurons().call(
            {"from": self.web3Connection.eth.accounts[accountNR]})
    ...
    def checkRoundTime(self, accountNR):
        thxHash = self.FLcontractDeployed.functions.initNewRound().transact(
            {"from": self.web3Connection.eth.accounts[accountNR]})
        self.__await_Trainsaction(thxHash)
