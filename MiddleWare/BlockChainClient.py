...
class BlockChainConnection:
    ...
    def __update_with_proof(self,weights,bias,accountNR,Device_name, round, account_number, proof):
    ...
    def __update_without_proof(self,weights,bias,accountNR, Device_name, round, account_number):
        weights = [[int(x) for x in y] for y in weights]
        bias = [int(x) for x in bias]
        IPFSdata = self.config["DEFAULT"]["IPFSDataPath"] + Device_name + '.json'
        IPFSFileHash = "0x00000000000000000000000000000000"
    ...
    def update(self,weights,bias,accountNR,Device_name, round, account_number, proof=None):
        if self.config["DEFAULT"]["PerformProof"]:
            tries=5
            IPFSFileHash = ''
            while tries>0:
                try:
                    IPFSFileHash = self.__update_with_proof(weights,bias,accountNR,Device_name, round, account_number,proof)
                    tries=-1
                except:
            ...
            while tries > 0:
                try:
                    IPFSFileHash = self.__update_without_proof(weights, bias, accountNR, Device_name, round, account_number)
                    tries = -1
                except:
                    time.sleep(self.config["DEFAULT"]["WaitingTime"])
                    if tries == 1:
